export function createListOmieProductsWithStockUseCase(deps: {
  omieProductReadRepo: {
  ...
  return {
    async execute(): Promise<{
      ...
      for (const row of stockRows) {
        ...
        const candidate: Date | null =
          (row.updatedAt ?? row.capturedAt) ?? null;

        if (
          candidate &&
          (!latestUpdatedAt ||
            candidate.getTime() > latestUpdatedAt.getTime())
        ) {
      ...
      return {
        items: enriched,
        stockUpdatedAt: latestUpdatedAt
